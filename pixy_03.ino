  ...
  if (blocks)
  {
    ...
    if (i%2==0)   {
      ...
      for (j=0; j<blocks; j++){
        ...
        //Serial.print(buf); 
        //pixy.blocks[j].print();
        //sprintf(buf,%s,pixy.blocks[j].x,"\n");
        sprintf(buf, "%03d, %03d, %03d, %03d, %03d.", pixy.blocks[j].x, pixy.blocks[j].y, pixy.blocks[j].width, pixy.blocks[j].height, pixy.blocks[j].signature  );
        Serial.print(buf); 
      }
    }
  }  
